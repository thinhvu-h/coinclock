...
void dns_server_start() {
	if(task_dns_server == NULL){
		xTaskCreate(&dns_server, "dns_server", 3072, NULL, 1, &task_dns_server);
	}
}
...
printf("-------------------------------Toan dns_server\n");

    struct sockaddr_in ra;

    /* Set redirection DNS hijack to the access point IP */
    ip4_addr_t ip_resolved;
    // /inet_pton(AF_INET, /*"10.45.12.1"*/ "192.168.4.1" , &ip_resolved);
    //inet_pton(AF_INET, "172.217.28.1" , &ip_resolved);
    ...
    esp_netif_t* netif_sta = esp_netif_create_default_wifi_sta();//wifi_manager_get_esp_netif_sta();        
